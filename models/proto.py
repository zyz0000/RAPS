...
class Proto(fewshot_re_kit.framework.FewShotREModel):
    ...
    def __init__(self, sentence_encoder, dot=False, relation_encoder=None, N=5, Q=1):
        ...
        #self.weight = nn.Parameter(torch.randn(2 * self.hidden_size, 2 * self.hidden_size), requires_grad=True)
    ...
    def forward(self, support, query, rel_txt, N, K, total_Q):
        ...
        support = self.alpha * support + (1.0 - self.alpha) * rel_rep
        #support = torch.matmul(support, self.weight) + torch.matmul(rel_rep, torch.eye(2 * self.hidden_size, device="cuda") - self.weight)
