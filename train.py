...
def main():
    ...
    parser.add_argument('--max_length', default=128, type=int,
                        help='max length')
    parser.add_argument('--num_kernels', default=256, type=int,
                        help='num kernels')
    ...
    # encoder
    sentence_encoder = BERTSentenceEncoder(opt.pretrain_ckpt, max_length, 
                       num_kernels=opt.num_kernels, path=opt.path)
    ...
    print("RESULT: %.2f +/- %.2f" % (np.mean(acc), np.std(acc)))
