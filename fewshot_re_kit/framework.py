    ...
    def train(self,
        ...
        for it in range(start_iter, start_iter + train_iter):
            ...
            if torch.cuda.is_available():
                ...
                label = label.cuda()
            logits, pred = model(support, query, rel_text, N_for_train, K, Q * N_for_train)
            ...
            if self.adv:
                ...
                features_ori, _ = model.sentence_encoder(support, cat=False)
                features_adv, _ = model.sentence_encoder(support_adv, cat=False)
            ...
            if self.adv:
                ...
                sys.stdout.write('step: {0:4} | loss: {1:2.6f}, accuracy: {2:3.2f}%'.
                                 format(it + 1, iter_loss / iter_sample, 100 * iter_right / iter_sample) + '\r')
            sys.stdout.flush()
            ...
            for it in range(eval_iter):
                ...
                if torch.cuda.is_available():
                    ...
                    for k in rel_text:
                        rel_text[k] = rel_text[k].cuda()    
                    label = label.cuda()
                logits, pred = model(support, query, rel_text, N, K, Q * N)
