...
class FewShotREModel(nn.Module):
    def __init__(self, sentence_encoder):
        ...
        self.cost = nn.CrossEntropyLoss(reduction='mean')
    ...
    def train(self,
        ...
        for it in range(start_iter, start_iter + train_iter):
            ...
            if torch.cuda.is_available():
                ...
                label = label.cuda()
            logits, pred, logits_proto, labels_proto = model(support, query, rel_text, N_for_train, K, Q * N_for_train)
            loss = model.loss(logits, label) / float(grad_iter) + lamda * self.bce_loss(logits_proto, labels_proto)
            ...
            if (it + 1) % val_step == 0:
                acc = np.mean(self.eval(model, N_for_eval, K, Q, val_iter))
                model.train()
                if acc > best_acc:
            ...
            for it in range(eval_iter):
                ...
                if torch.cuda.is_available():
                    ...
                    for k in rel_text:
                        rel_text[k] = rel_text[k].cuda()
                    label = label.cuda()
                logits, pred, _, _ = model(support, query, rel_text, N, K, Q * N, is_eval=True)
                ...
                sys.stdout.flush()
                acc.append(100 * right.data)
            print("")
        #return iter_right / iter_sample, acc
        return acc
